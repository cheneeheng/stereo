  ...
  _FloatWindow grady)      /*   " */
{
  ...
  for (j = -hh ; j <= hh ; j++)
    for (i = -hw ; i <= hw ; i++)  {
      g1 = _KLTInterpolate(x1+i, y1+j, gradx1);
      g2 = _KLTInterpolate(x2+i, y2+j, gradx2);
      *gradx++ = g1 + g2;
      g1 = _KLTInterpolate(x1+i, y1+j, grady1);
      g2 = _KLTInterpolate(x2+i, y2+j, grady2);
      *grady++ = g1 + g2;
    }
}
  ...
  _FloatWindow imgdiff)   /* output */
{
  ...
  if(mean2==0 || mean1==0) // 2nd patch is black, thus alpha=Inf and it crashes
  {
  ...
  _FloatWindow grady)      /*   " */
{
  ...
  for (j = -hh ; j <= hh ; j++)
    for (i = -hw ; i <= hw ; i++)  {
      g1 = _KLTInterpolate(x1+i, y1+j, img1);
      g2 = _KLTInterpolate(x2+i, y2+j, img2);
      sum1_squared += g1;    sum2_squared += g2; // isn't this a BUG? cf. function above
    }
  mean1 = sum1_squared/(width*height); // this should be copied from the function above
  mean2 = sum2_squared/(width*height); // this should be copied from the function above
  alpha = (float) sqrt(mean1/mean2);   // this should be copied from the function above
  ...
  float *ey)
{
  ...
  *ex = 0;  *ey = 0;
  for (i = 0 ; i < width * height ; i++)  {
      diff = *imgdiff++; // better: diff = *(imgdiff+i);
      *ex += 2 * diff * (*gradx++);
      *ey += 2 * diff * (*grady++);
  }
}
...
int _KLTTrackFeature(
  ...
  float *residue)	 /*sum of residue gray difference in tracked patch*/
{
  ...
  do  {
		/*fprintf(stderr,  "Hier 1, x1: %f, x2: %f, y1: %f, y2: %f\n", x1, *x2, y1, *y2);
		fprintf(stderr,  "hw: %d, nc: %d, hh: %d\n", hw, nc, hh);
		fflush(stderr);*/
  ...
  if (status == KLT_TRACKED)  {
    if (lighting_insensitive)
      ...
      _computeIntensityDifference(img1, img2, x1, y1, *x2, *y2,
                                  width, height, imgdiff);
    if(residue){
      *residue=_sumAbsFloatWindow(imgdiff, width, height)/(width*height);
      //fprintf(stderr,"residue: %f, max_residue: %f, status: %d\n",residue, max_residue, status);
				     ...
				     _FloatWindow out_grady)      /* output */
{
  ...
  for (j = -hh ; j <= hh ; j++)
    for (i = -hw ; i <= hw ; i++)  {
      mi = Axx * i + Axy * j;
      mj = Ayx * i + Ayy * j;
      *out_gradx++ = _KLTInterpolate(x+mi, y+mj, in_gradx);
      *out_grady++ = _KLTInterpolate(x+mi, y+mj, in_grady);
    }
					 ...
					 _FloatWindow imgdiff)   /* output */
{
  ...
  for (j = -hh ; j <= hh ; j++)
    for (i = -hw ; i <= hw ; i++)  {
      mi = Axx * i + Axy * j;
      mj = Ayx * i + Ayy * j;
      *imgdiff++ = _KLTInterpolate(x+mi, y+mj, img);
    }
}
						 ...
						 _FloatWindow imgdiff)   /* output */
{
  ...
  for (j = -hh ; j <= hh ; j++)
    for (i = -hw ; i <= hw ; i++)  {
      ...
      mi = Axx * i + Axy * j;
      mj = Ayx * i + Ayy * j;
      g2 = _KLTInterpolate(x2+mi, y2+mj, img2);
      *imgdiff++ = g1 - g2;
    }
}
...
static int _trackFeatureAffine(
				  float x1,  /* location of window in first image */
				  float y1,
				  float *x2, /* starting location of search in second image */
				  float *y2,
...
static void _freeAffineImages(KLT_Feature feature)
{
	if( feature-> aff_img ) _KLTFreeFloatImage(feature->aff_img);
	if( feature->aff_img_gradx ) _KLTFreeFloatImage(feature->aff_img_gradx);
	if( feature->aff_img_grady ) _KLTFreeFloatImage(feature->aff_img_grady);
	feature->aff_img = NULL;
	feature->aff_img_gradx = NULL;
	feature->aff_img_grady = NULL;
...
static void _KLTStdTrackFeatures(
					  ...
					  KLT_FeatureList featurelistDest)
{
	...
	float xloc, yloc, xlocout, ylocout, old_x, old_y;
	int val;
	int indx, r;
	KLT_BOOL floatimg1_created = FALSE;
	int i;
	int nFeatures = featurelistSrc->nFeatures;
	...
	if (KLT_verbose >= 1)  {
		fprintf(stderr,  "(KLT) Tracking %d features in a %d by %d image...  ",
			KLTCountRemainingFeatures(featurelistSrc), ncols, nrows);
		fflush(stderr);
	}

	//check length of feature lists
	if(nFeatures != featurelistDest->nFeatures)
	{
		KLTError("The length of the feature lists is not equal!");
	}
	...
	if (tc->sequentialMode==2 && tc->pixelDisplacements_last==NULL) //allocate memory to add previous pixel displacement
	{
		tc->pixelDisplacements_last = (_KLT_FloatPixelDispRec *)malloc(nFeatures*sizeof(_KLT_FloatPixelDispRec));
	}
	...
	if (tc->sequentialMode && tc->pyramid_last != NULL)  {
		...
		_KLTComputeSmoothedImage(tmpimg, _KLTComputeSmoothSigma(tc), &gauss_kernel, &gaussderiv_kernel, &sigma_last, floatimg1);
		pyramid1 = _KLTCreatePyramid(ncols, nrows, (int) subsampling, tc->nPyramidLevels);
		_KLTComputePyramid(floatimg1, pyramid1, tc->pyramid_sigma_fact, &gauss_kernel, &gaussderiv_kernel, &sigma_last);
	...
	_KLTComputeSmoothedImage(tmpimg, _KLTComputeSmoothSigma(tc), &gauss_kernel, &gaussderiv_kernel, &sigma_last, floatimg2);
	pyramid2 = _KLTCreatePyramid(ncols, nrows, (int) subsampling, tc->nPyramidLevels);
	_KLTComputePyramid(floatimg2, pyramid2, tc->pyramid_sigma_fact, &gauss_kernel, &gaussderiv_kernel, &sigma_last);
	...
	/* For each feature, do ... */
	for (indx = 0 ; indx < featurelistSrc->nFeatures ; indx++)  {

		/* Only track features that are not lost */
		if (featurelistSrc->feature[indx]->val >= 0)  {

			old_x=featurelistSrc->feature[indx]->x;
			old_y=featurelistSrc->feature[indx]->y;

			xloc = old_x;
			yloc = old_y;

			xlocout = featurelistDest->feature[indx]->x; //here the new position is used
			ylocout = featurelistDest->feature[indx]->y; // "
			...
			for (r = tc->nPyramidLevels - 1 ; r >= 0 ; r--)  {
				...
				//fprintf(stderr,"indx: %d, r: %d, xloc: %f, xlocout: %f, yloc: %f, ylocout: %f\n", indx, r, xloc, xlocout, yloc, ylocout);
				//fflush(stderr);

				val = _KLTTrackFeature(xloc, yloc,
					&xlocout, &ylocout,
					pyramid1->img[r],
			...
			/* Record feature */
			if (val == KLT_OOB) {
				featurelistDest->feature[indx]->x   = -1.0;
				featurelistDest->feature[indx]->y   = -1.0;
				featurelistDest->feature[indx]->val = KLT_OOB;
				//_freeAffineImages(featurelistDest->feature[indx]);
				...
				featurelistDest->feature[indx]->x   = -1.0;
				featurelistDest->feature[indx]->y   = -1.0;
				featurelistDest->feature[indx]->val = KLT_SMALL_DET;
				//_freeAffineImages(featurelistDest->feature[indx]);
			} else if (val == KLT_LARGE_RESIDUE)  {
				featurelistDest->feature[indx]->x   = -1.0;
				featurelistDest->feature[indx]->y   = -1.0;
				featurelistDest->feature[indx]->val = KLT_LARGE_RESIDUE;
				//_freeAffineImages(featurelistDest->feature[indx]);
			} else if (val == KLT_MAX_ITERATIONS)  {
				featurelistDest->feature[indx]->x   = -1.0;
				featurelistDest->feature[indx]->y   = -1.0;
				featurelistDest->feature[indx]->val = KLT_MAX_ITERATIONS;
				//_freeAffineImages(featurelistDest->feature[indx]);
			} else  {
				featurelistDest->feature[indx]->x = xlocout;
				featurelistDest->feature[indx]->y = ylocout;
				featurelistDest->feature[indx]->val = KLT_TRACKED;
				if (tc->affineConsistencyCheck >= 0 && val == KLT_TRACKED)  { /*for affine mapping*/
					...
					if(!featurelistDest->feature[indx]->aff_img){
						/* save image and gradient for each feature at finest resolution after first successful track */
						featurelistDest->feature[indx]->aff_img = _KLTCreateFloatImage((tc->affine_window_width+border), (tc->affine_window_height+border));
						featurelistDest->feature[indx]->aff_img_gradx = _KLTCreateFloatImage((tc->affine_window_width+border), (tc->affine_window_height+border));
						featurelistDest->feature[indx]->aff_img_grady = _KLTCreateFloatImage((tc->affine_window_width+border), (tc->affine_window_height+border));
						_am_getSubFloatImage(pyramid1->img[0],xloc,yloc,featurelistDest->feature[indx]->aff_img);
						_am_getSubFloatImage(pyramid1_gradx->img[0],xloc,yloc,featurelistDest->feature[indx]->aff_img_gradx);
						_am_getSubFloatImage(pyramid1_grady->img[0],xloc,yloc,featurelistDest->feature[indx]->aff_img_grady);
						featurelistDest->feature[indx]->aff_x = xloc - (int) xloc + (tc->affine_window_width+border)/2;
						featurelistDest->feature[indx]->aff_y = yloc - (int) yloc + (tc->affine_window_height+border)/2;;
					}else{
						/* affine tracking */
						val = _trackFeatureAffine(featurelistDest->feature[indx]->aff_x, featurelistDest->feature[indx]->aff_y,
							&xlocout, &ylocout,
							featurelistDest->feature[indx]->aff_img,
							featurelistDest->feature[indx]->aff_img_gradx,
							featurelistDest->feature[indx]->aff_img_grady,
							pyramid2->img[0],
							...
							&featurelistDest->feature[indx]->aff_Axx,
							&featurelistDest->feature[indx]->aff_Ayx,
							&featurelistDest->feature[indx]->aff_Axy,
							&featurelistDest->feature[indx]->aff_Ayy
							);
						featurelistDest->feature[indx]->val = val;
						if(val != KLT_TRACKED){
							featurelistDest->feature[indx]->x   = -1.0;
							featurelistDest->feature[indx]->y   = -1.0;
							featurelistDest->feature[indx]->aff_x = -1.0;
							featurelistDest->feature[indx]->aff_y = -1.0;
			...
			if (tc->sequentialMode==2 && val==KLT_TRACKED)  {
				tc->pixelDisplacements_last[indx].xDisp = featurelistDest->feature[indx]->x - old_x;
				tc->pixelDisplacements_last[indx].yDisp = featurelistDest->feature[indx]->y - old_y;
			}
	...
	if (KLT_verbose >= 1)  {
		fprintf(stderr,  "\n\t%d features successfully tracked.\n",
			KLTCountRemainingFeatures(featurelistDest));
		if (tc->writeInternalImages)
...
static void _bindCorner( _KLT_FloatImage gradx,
						 ...
						 float *ylocout)
{
	...
	int new_x, new_y;
	float center_val;
	int x_center=roundf(*xlocout);
	int y_center=roundf(*ylocout);
	register float gx, gy;
	int window_hw = window_width/2;
	int window_hh = window_height/2;
	register int xx, yy;
	int x, y;
	float val;
    ...
    for (y = y_center-1 ; y <= y_center + 1 ; y++)
      for (x = x_center-1 ; x <= x_center+1 ; x++)  {
        ...
        register float gxx = 0; register float gxy = 0; register float gyy = 0;
        for (yy = y-window_hh ; yy <= y+window_hh ; yy++)
          for (xx = x-window_hw ; xx <= x+window_hw ; xx++)  {
            gx = *(gradx->data + roi_width*yy+xx);
            gy = *(grady->data + roi_width*yy+xx);
            gxx += gx * gx;
            gxy += gx * gy;
            gyy += gy * gy;
          }
        ...
        if (val > limit)  {
          KLTWarning("(_KLTCheckGoodFeatures) minimum eigenvalue %f is "
    ...
    float factor = center_val/result;
    //factor *= factor; //square relation
    factor = 1.0-factor;
    //fprintf(stderr, "x: %f, y: %f, factor: %f\n", (float)(new_x-x_center)*factor, (float)(new_y-y_center)*factor, factor);
    *xlocout+=(float)(new_x-x_center)*factor;
    *ylocout+=(float)(new_y-y_center)*factor;
...
void _KLTExtTrackFeatures(
					  ...
					  KLT_FeatureList featurelistDest)
{
	_KLT_FloatImage floatimg, tmpimg;
	float subsampling = (float) tc->subsampling;
	float xloc, yloc, xlocout, ylocout;
	int val;
	int indx, r;
	int i;
	int roiSrc_width  = tc->window_width+2*tc->borderx;
	int roiSrc_height = tc->window_height+2*tc->bordery;
	int roiDest_width  = roiSrc_width  + 2*tc->max_patchDisplacement;
	int roiDest_height = roiSrc_height + 2*tc->max_patchDisplacement;
	_KLT_Pyramid pyramid1=NULL, pyramid1_gradx, pyramid1_grady,
			pyramid2=NULL, pyramid2_gradx, pyramid2_grady;
	IppiRect *roi1, *roi2;
	ConvolutionKernel gauss_kernel;
	ConvolutionKernel gaussderiv_kernel;
	float sigma_last = -10.0;
	...
	//extended things
	//KLT_BOOL newPatchList = FALSE;
	KLT_BOOL comparingToWholeImage = FALSE;
	int nFeatures=featurelistSrc->nFeatures;
	//IppiRect *roilistSrc[nFeatures];//=(IppiRect **)malloc(nFeatures*sizeof(IppiRect *));
	//IppiRect *roilistDest[nFeatures];//=(IppiRect **)malloc(nFeatures*sizeof(IppiRect *));

	if (KLT_verbose >= 1)  {
		fprintf(stderr,  "(KLT) Tracking %d features in a %d by %d image...  \n",
				KLTCountRemainingFeatures(featurelistSrc), ncols, nrows);
		fprintf(stderr,  "(KLT_EXT) PATCH VERSION\n");
		fflush(stderr);
	}
	...
	if (tc->sequentialMode)
	{
/*		//int help = tc->sequentialMode;
		if(tc->patchlist_last != NULL && tc->patchlist_last->nFeatures != nFeatures)
		{
			KLTFreeStoredImages(tc);
			//KLTStopSequentialMode(tc);
			//tc->sequentialMode = help;
		}
		if(tc->patchlist_last == NULL)
		{
			tc->patchlist_last = (_KLT_PatchList)malloc(sizeof(_KLT_PatchListRec));
			tc->patchlist_last->patches = (_KLT_PatchRec *)malloc(nFeatures*sizeof(_KLT_PatchRec));
			tc->patchlist_last->nFeatures = nFeatures;
			newPatchList = TRUE;
		}
*/
		if (tc->sequentialMode==2 && tc->pixelDisplacements_last==NULL) //allocate memory to add previous pixel displacement
	...
	if(tc->sequentialMode && tc->pyramid_last!=NULL) //for the case that the patches have to be compared to the whole image
	{
		comparingToWholeImage = TRUE;
		pyramid1 = tc->pyramid_last;
		pyramid1_gradx = tc->pyramid_last_gradx;
		pyramid1_grady = tc->pyramid_last_grady;
		roi1 = (IppiRect *)malloc(sizeof(IppiRect));
		roi1->x=0;
		roi1->y=0;
		roi1->width=ncols;
		roi1->height=nrows;
	...
	for (indx = 0 ; indx < featurelistSrc->nFeatures ; indx++)  {
		...
		/* Only track features that are not lost */
		KLT_Feature featureSrc = featurelistSrc->feature[indx];
		KLT_Feature featureDest = featurelistDest->feature[indx];
		KLT_Feature featureResult = featureDest; //to make it easier to change the result location
		int featureVal=featureDest->val;
		_KLT_Patch patch=NULL;

		if (featureVal >= 0)  {
			KLT_locType old_x, old_y;

			if(!comparingToWholeImage)
			{
				if(featureSrc->patch && (tc->nPyramidLevels != featureSrc->patch->pyramid->nLevels) ) //delete Patch, if pyramid doesn't match
				{
					_KLTFreePatch(featureSrc->patch);
					featureSrc->patch=NULL;
				}

				if(tc->sequentialMode && featureSrc->patch)
				{
					pyramid1 = featureSrc->patch->pyramid;
					pyramid1_gradx = featureSrc->patch->pyramid_gradx;
					pyramid1_grady = featureSrc->patch->pyramid_grady;
					roi1 = featureSrc->patch->roi;
				}
				else
				{
					roi1 =_KLTComputePatch(roiSrc_width, roiSrc_height, featureSrc, ncols, nrows);
					int width=roi1->width;
					int height=roi1->height;
					tmpimg = _KLTCreateFloatImage(width, height);
					floatimg = _KLTCreateFloatImage(width, height);
					_KLTPatchToFloatImage(img1, ncols, nrows, roi1, tmpimg);
					_KLTComputeSmoothedImage(tmpimg, _KLTComputeSmoothSigma(tc), &gauss_kernel, &gaussderiv_kernel, &sigma_last, floatimg);
					pyramid1 = _KLTCreatePyramid(width, height, (int) subsampling, tc->nPyramidLevels);
					_KLTComputePyramid(floatimg, pyramid1, tc->pyramid_sigma_fact, &gauss_kernel, &gaussderiv_kernel, &sigma_last);
					pyramid1_gradx = _KLTCreatePyramid(width, height, (int) subsampling, tc->nPyramidLevels);
					pyramid1_grady = _KLTCreatePyramid(width, height, (int) subsampling, tc->nPyramidLevels);
					for (i = 0 ; i < tc->nPyramidLevels ; i++)
						_KLTComputeGradients(pyramid1->img[i], tc->grad_sigma,
								&gauss_kernel, &gaussderiv_kernel, &sigma_last,
								pyramid1_gradx->img[i],
			...
			/*if (tc->sequentialMode && !newPatchList && featureVal==0)  {
				pyramid1 = tc->patchlist_last->patches[indx].pyramid;
				pyramid1_gradx = tc->patchlist_last->patches[indx].pyramid_gradx;
				pyramid1_grady = tc->patchlist_last->patches[indx].pyramid_grady;
				roi1 = tc->patchlist_last->patches[indx].roi;
			} else if(!comparingToWholeImage) {
				//roi1=roilistSrc[indx];
				roi1 =_KLTComputePatch(roiSrc_width, roiSrc_height, featureSrc, ncols, nrows);
				int width=roi1->width;
				int height=roi1->height;
				tmpimg = _KLTCreateFloatImage(width, height);
				...
				pyramid1 = _KLTCreatePyramid(width, height, (int) subsampling, tc->nPyramidLevels);
				_KLTComputePyramid(floatimg, pyramid1, tc->pyramid_sigma_fact);
				pyramid1_gradx = _KLTCreatePyramid(width, height, (int) subsampling, tc->nPyramidLevels);
				pyramid1_grady = _KLTCreatePyramid(width, height, (int) subsampling, tc->nPyramidLevels);
				for (i = 0 ; i < tc->nPyramidLevels ; i++)
					_KLTComputeGradients(pyramid1->img[i], tc->grad_sigma,
			...
			//roi2=roilistDest[indx];
			roi2=_KLTComputePatch(roiDest_width, roiDest_height, featureDest, ncols, nrows);
			int width=roi2->width;
			int height=roi2->height;
			// Do the same thing with second image
			tmpimg = _KLTCreateFloatImage(width, height);
			floatimg = _KLTCreateFloatImage(width, height);
			_KLTPatchToFloatImage(img2, ncols, nrows, roi2, tmpimg);
			_KLTComputeSmoothedImage(tmpimg, _KLTComputeSmoothSigma(tc), &gauss_kernel, &gaussderiv_kernel, &sigma_last, floatimg);
			pyramid2 = _KLTCreatePyramid(width, height, (int) subsampling, tc->nPyramidLevels);
			_KLTComputePyramid(floatimg, pyramid2, tc->pyramid_sigma_fact, &gauss_kernel, &gaussderiv_kernel, &sigma_last);
			pyramid2_gradx = _KLTCreatePyramid(width, height, (int) subsampling, tc->nPyramidLevels);
			pyramid2_grady = _KLTCreatePyramid(width, height, (int) subsampling, tc->nPyramidLevels);
			for (i = 0 ; i < tc->nPyramidLevels ; i++)
				_KLTComputeGradients(pyramid2->img[i], tc->grad_sigma,
					&gauss_kernel, &gaussderiv_kernel, &sigma_last,
					pyramid2_gradx->img[i],
			...
			/*if (tc->writeInternalImages)  {
				...
				static int first=1;
				for (i = 0 ; i < tc->nPyramidLevels ; i++)  {
					if(!comparingToWholeImage || first)
					{
						sprintf(fname, "%sp%d_kltimg_tf_i%d.pgm", dir, indx, i);
						_KLTWriteFloatImageToPGM(pyramid1->img[i], fname);
						sprintf(fname, "%sp%d_kltimg_tf_i%d_gx.pgm", dir, indx, i);
						_KLTWriteFloatImageToPGM(pyramid1_gradx->img[i], fname);
						sprintf(fname, "%sp%d_kltimg_tf_i%d_gy.pgm", dir, indx, i);
						_KLTWriteFloatImageToPGM(pyramid1_grady->img[i], fname);
					}
					sprintf(fname, "%sp%d_kltimg_tf_j%d.pgm", dir, indx, i);
					_KLTWriteFloatImageToPGM(pyramid2->img[i], fname);
					sprintf(fname, "%sp%d_kltimg_tf_j%d_gx.pgm", dir, indx, i);
					_KLTWriteFloatImageToPGM(pyramid2_gradx->img[i], fname);
					sprintf(fname, "%sp%d_kltimg_tf_j%d_gy.pgm", dir, indx, i);
					_KLTWriteFloatImageToPGM(pyramid2_grady->img[i], fname);
				}
				first=0;
			}*/
			...
			xlocout = featureDest->x - roi2->x;
			ylocout = featureDest->y - roi2->y;


			//fprintf(stderr, "\nf: %d\nxloc1: %f\nxloc2: %f\nyloc1: %f\nyloc2:%f\n", indx, xloc,xlocout,yloc,ylocout);
			//fflush(stderr);
			...
			for (r = tc->nPyramidLevels - 1 ; r >= 0 ; r--)  {
				...
				//fprintf(stderr, "xloc2: %f \t yloc2:%f \t %d\n", xlocout,ylocout, val);
			...
			if (val == KLT_OOB) {
				featureResult->x   = -1.0;
				featureResult->y   = -1.0;
				featureResult->val = KLT_OOB;
				//_freeAffineImages(featureResult);
			} else if (_outOfBounds(xlocout, ylocout, roi2->width, roi2->height, tc->borderx, tc->bordery))  {
//			} else if (_outOfBounds(xlocout, ylocout, ncols, nrows, tc->borderx, tc->bordery))  {
				featureResult->x   = -1.0;
				featureResult->y   = -1.0;
				featureResult->val = KLT_OOB;
				...
				featureResult->x   = -1.0;
				featureResult->y   = -1.0;
				featureResult->val = KLT_SMALL_DET;
				//_freeAffineImages(featureResult);
			} else if (val == KLT_LARGE_RESIDUE)  {
				featureResult->x   = -1.0;
				featureResult->y   = -1.0;
				featureResult->val = KLT_LARGE_RESIDUE;
				//_freeAffineImages(featureResult);
			} else if (val == KLT_MAX_ITERATIONS)  {
				featureResult->x   = -1.0;
				featureResult->y   = -1.0;
				featureResult->val = KLT_MAX_ITERATIONS;
				//_freeAffineImages(featureResult);
			} else  {


				if(tc->bindToCorner)
					_bindCorner( 	pyramid2_gradx->img[0],
									pyramid2_grady->img[0],
									roi2->width,
									tc->window_width,
									tc->window_height,
									&xlocout,
				...
				featureResult->x = xlocout + roi2->x;
				featureResult->y = ylocout + roi2->y;
				//featurelist->feature[indx]->x = xlocout;
				//featurelist->feature[indx]->y = ylocout;
				featureResult->val = KLT_TRACKED;
				if (tc->affineConsistencyCheck >= 0 && val == KLT_TRACKED)  { //for affine mapping
					int border = 2; // add border for interpolation
					...
					if(!featureSrc->aff_img){
						// save image and gradient for each feature at finest resolution after first successful track
						featureResult->aff_img = _KLTCreateFloatImage((tc->affine_window_width+border), (tc->affine_window_height+border));
						featureResult->aff_img_gradx = _KLTCreateFloatImage((tc->affine_window_width+border), (tc->affine_window_height+border));
						featureResult->aff_img_grady = _KLTCreateFloatImage((tc->affine_window_width+border), (tc->affine_window_height+border));
						_am_getSubFloatImage(pyramid1->img[0],xloc,yloc,featureResult->aff_img);
						_am_getSubFloatImage(pyramid1_gradx->img[0],xloc,yloc,featureResult->aff_img_gradx);
						_am_getSubFloatImage(pyramid1_grady->img[0],xloc,yloc,featureResult->aff_img_grady);
						featureResult->aff_x = xloc - (int) xloc + (tc->affine_window_width+border)/2;
						featureResult->aff_y = yloc - (int) yloc + (tc->affine_window_height+border)/2;;
					}else{
						// affine tracking
						/*featureResult->aff_img = featureSrc->aff_img;
						featureResult->aff_img_gradx = featureSrc->aff_img_gradx;
						featureResult->aff_img_grady = featureSrc->aff_img_grady;
						featureSrc->aff_img = NULL;
						featureSrc->aff_img_gradx = NULL;
						featureSrc->aff_img_grady = NULL;*/

						val = _trackFeatureAffine(featureResult->aff_x, featureResult->aff_y,
							&xlocout, &ylocout,
							featureSrc->aff_img,
							featureSrc->aff_img_gradx,
							featureSrc->aff_img_grady,
							pyramid2->img[0],
							pyramid2_gradx->img[0], pyramid2_grady->img[0],
							tc->affine_window_width, tc->affine_window_height,
							tc->affine_max_iterations,
							...
							tc->affineConsistencyCheck,
							tc->affine_max_displacement_differ,
							&featureResult->aff_Axx,
							&featureResult->aff_Ayx,
							&featureResult->aff_Axy,
							&featureResult->aff_Ayy
							);
						featureResult->val = val;
						if(val != KLT_TRACKED){
							featureResult->x   = -1.0;
							featureResult->y   = -1.0;
							featureResult->aff_x = -1.0;
							featureResult->aff_y = -1.0;
							// free image and gradient for lost feature
							//_freeAffineImages(featureResult);
						}else{
							//featureResult->x = xlocout + roi2->x;
			...
			if (tc->writeInternalImages && (tc->writeInternalImages<10 || tc->writeInternalImages==(indx+10)) )  {
				char fname[80];
				char dir[]="InternalImages/";
				static int run=0;
				for (i = 0 ; i < tc->nPyramidLevels ; i++)
				{
					if(tc->writeInternalImages == 1)
					{
						if(!comparingToWholeImage || !run)
						{
							sprintf(fname, "%sp%d_kltimg_tf_i%d.pgm", dir, indx, i);
							_KLTWriteFloatImageToPGM(pyramid1->img[i], fname);
							sprintf(fname, "%sp%d_kltimg_tf_i%d_gx.pgm", dir, indx, i);
					...
					else if(i==0) //save only high resolution images
					{
						if(!comparingToWholeImage || !run)
						{
							if(i==0){ //only within high resolution images
								sprintf(fname, "%sp%d_kltimg_tf_i%d_n%d.ppm", dir, indx, i, run);
								_KLTWritePatchToPPM(pyramid1->img[i], fname, xloc, yloc, tc->window_width, tc->window_height, val==KLT_TRACKED ? 1 : 0);
							} else{
								sprintf(fname, "%sp%d_kltimg_tf_i%d_n%d.pgm", dir, indx, i, run);
								_KLTWriteFloatImageToPGM(pyramid1->img[i], fname);
							}
							sprintf(fname, "%sp%d_kltimg_tf_i%d_n%d_gx.pgm", dir, indx, i, run);
							_KLTWriteFloatImageToPGM(pyramid1_gradx->img[i], fname);
							sprintf(fname, "%sp%d_kltimg_tf_i%d_n%d_gy.pgm", dir, indx, i, run);
							_KLTWriteFloatImageToPGM(pyramid1_grady->img[i], fname);
						}
						if(i==0) { //only within high resolution images
							sprintf(fname, "%sp%d_kltimg_tf_j%d_n%d.ppm", dir, indx, i, run);
							_KLTWritePatchToPPM(pyramid2->img[i], fname, xlocout, ylocout, tc->window_width, tc->window_height, val==KLT_TRACKED ? 1 : 0);
						} else{
							sprintf(fname, "%sp%d_kltimg_tf_j%d_n%d.pgm", dir, indx, i, run);
							_KLTWriteFloatImageToPGM(pyramid2->img[i], fname);
						}
						sprintf(fname, "%sp%d_kltimg_tf_j%d_n%d_gx.pgm", dir, indx, i, run);
						_KLTWriteFloatImageToPGM(pyramid2_gradx->img[i], fname);
						sprintf(fname, "%sp%d_kltimg_tf_j%d_n%d_gy.pgm", dir, indx, i, run);
						_KLTWriteFloatImageToPGM(pyramid2_grady->img[i], fname);
					}
				}
				run++;
			...
			if (tc->sequentialMode && val==KLT_TRACKED)  {
                            
				// save new patch
				patch=_KLTCreatePatch(); //allocate memory
				patch->pyramid = pyramid2;
				patch->pyramid_gradx = pyramid2_gradx;
				patch->pyramid_grady = pyramid2_grady;
				patch->roi = roi2;
				featureResult->patch=patch;
                                
                                // save pixelDisplacements
				if (tc->sequentialMode==2 )  {
                                        // don't save if just refound
                                        if(featureVal==KLT_REFIND){// && val==KLT_TRACKED){
                                            tc->pixelDisplacements_last[indx].xDisp = 0.;
                                            tc->pixelDisplacements_last[indx].yDisp = 0.;
                                        }
                                        else{
                                            tc->pixelDisplacements_last[indx].xDisp = featureResult->x - old_x;
                                            tc->pixelDisplacements_last[indx].yDisp = featureResult->y - old_y;
                                        }
                                }
			} else  {
				_KLTFreePyramid(pyramid2);
				...
				free(roi2);
				//if(tc->sequentialMode)
				//	featureDest->patch=NULL;
			}

			/*if(!comparingToWholeImage) //don't remove src images cause they could be needed
			{
				_KLTFreePyramid(pyramid1);
				_KLTFreePyramid(pyramid1_gradx);
				_KLTFreePyramid(pyramid1_grady);
		...
		}//end if(feature[indx]->val > 0)
		/*else if(newPatchList)
		{
			tc->patchlist_last->patches[indx].pyramid = NULL; //initialisierung, damit FreeTrackingContext bescheid weiß
		}*/
	...
	if(comparingToWholeImage)
	{
		...
		_KLTFreePyramid(pyramid1_gradx);
		_KLTFreePyramid(pyramid1_grady);
		tc->pyramid_last=NULL; // initialisierung, damit FreeTrackingContext bescheid weiß
					  ...
					  KLT_FeatureList featurelist)
{
	...
	KLT_FeatureList fl_help = KLTCreateFeatureList(nFeatures);

	if(tc->sequentialMode==2 && tc->pixelDisplacements_last != NULL)
	{
		if (KLT_verbose >= 1)  {
			fprintf(stderr,  "(KLT_EXT) Tracking with linear feature propagation: \n");
		}
		int j;

		for( j=0;j<nFeatures; j++)
		{
			KLT_Feature feature=featurelist->feature[j];
			KLT_Feature feature_help=fl_help->feature[j];
			if(feature->val==0)
			{
				...
				if(help>=tc->borderx && help<(ncols-tc->borderx))
					...
					feature_help->x = feature->x;
				help = feature->y + tc->pixelDisplacements_last[j].yDisp;
				//hWin = (float)tc->window_height/2.0;
				if(help>=tc->bordery && help<(nrows-tc->bordery))
					feature_help->y = help;
				else
					feature_help->y = feature->y;
			}
			feature_help->val = feature->val;
		}
        //KLTWriteFeatureListToPPM(fl_help, img2, ncols, nrows, "feat2_guessed.ppm");

	}
	else
		KLTCopyFeatureList(featurelist, fl_help);

	KLTExtTrackFeatures(tc, img1, img2, ncols, nrows, featurelist, fl_help);

	//if(tc->patchMode)
		KLTMoveFeatureList(fl_help, featurelist);
	KLTFreeFeatureList(fl_help);
 ...
 * KLTExtTrackFeatures
 *
 * Tracks feature points from one image to the next with a propagated feature list
 */
...
{
	if(tc->patchMode)
		_KLTExtTrackFeatures(tc, img1, img2, ncols, nrows, featurelistSrc, featurelistDest);
	else
		_KLTStdTrackFeatures(tc, img1, img2, ncols, nrows, featurelistSrc, featurelistDest);
}
