...
typedef enum {FEATURE_LIST, FEATURE_HISTORY, FEATURE_TABLE, DISPLACEMENT_LIST} structureType;
...
//	            KLTCountRemainingFeatures(featurelist), filename);
	  ...
	  /* Allocate memory for component images */
	  redimg = (uchar *)  malloc(2*nbytes);
	  grnimg = (uchar *)  malloc(2*nbytes);
	  bluimg = (uchar *)  malloc(2*nbytes);
	  if (redimg == NULL || grnimg == NULL || bluimg == NULL)
	    KLTError("(KLTWriteFeaturesToPPM)  Out of memory\n");
	  ...
	  if (sizeof(KLT_PixelType) != 1)
	    KLTWarning("(KLTWriteFeaturesToPPM)  KLT_PixelType is not uchar");	  
	  //left
	  memcpy(redimg, img_left, nbytes);
	  memcpy(grnimg, img_left, nbytes);
	  memcpy(bluimg, img_left, nbytes);
	  //right
	  memcpy(redimg+nbytes, img_right, nbytes);
	  memcpy(grnimg+nbytes, img_right, nbytes);
	  memcpy(bluimg+nbytes, img_right, nbytes);

	
	  /* Overlay features in red */
	  //left
	  for (i = 0 ; i < fl_left->nFeatures ; i++)
	    if (fl_left->feature[i]->val >= 0)  {
	      x = (int) (fl_left->feature[i]->x + 0.5);
	      y = (int) (fl_left->feature[i]->y + 0.5);
	      for (yy = y - 1 ; yy <= y + 1 ; yy++)
	        for (xx = x - 1 ; xx <= x + 1 ; xx++)  
	          if (xx >= 0 && yy >= 0 && xx < ncols && yy < nrows)  {
	            offset = yy * ncols + xx;
	  ...
	  //right
	  for (i = 0 ; i < fl_right->nFeatures ; i++)
	    if (fl_right->feature[i]->val >= 0)  {
	      x = (int) (fl_right->feature[i]->x + 0.5);
	      y = (int) (fl_right->feature[i]->y + 0.5);
	      for (yy = y - 1 ; yy <= y + 1 ; yy++)
	        for (xx = x - 1 ; xx <= x + 1 ; xx++)  
	          if (xx >= 0 && yy >= 0 && xx < ncols && yy < nrows)  {
	            offset = yy * ncols + xx + nbytes;
	            *(redimg + offset) = 255;
	            *(grnimg + offset) = 0;
	            *(bluimg + offset) = 0;
	          }
	  ...
	  //result
	  for (i = 0 ; i < fl_result->nFeatures ; i++)
	    if (fl_result->feature[i]->val >= 0)  {
	      x = (int) (fl_result->feature[i]->x + 0.5);
	      y = (int) (fl_result->feature[i]->y + 0.5);
	      for (yy = y - 1 ; yy <= y + 1 ; yy++)
	        for (xx = x - 1 ; xx <= x + 1 ; xx++)  
	          if (xx >= 0 && yy >= 0 && xx < ncols && yy < nrows)  {
	            offset = yy * ncols + xx + nbytes;
	  ...
	  for (i = 0 ; i < fl_result->nFeatures ; i++)
	    if (fl_result->feature[i]->val >= 0)  {
	      x1 = (int) (fl_left->feature[i]->x + 0.5);
	      y1 = (int) (fl_left->feature[i]->y + 0.5);
	      x2 = (int) (fl_result->feature[i]->x + 0.5);
	      y2 = (int) (fl_result->feature[i]->y + 0.5) + nrows;
    	  xd = (float)(x2-x1)/(float)(y2-y1);
    	  xs = x1;
    	  for (yy = y1 ; yy <= y2 ; yy++)
	      {
            offset = yy * ncols + (int)(xs+0.5);
            if(*(redimg + offset) == 255 && *(grnimg + offset) == 0 && *(bluimg + offset) == 0) //if a red point
            	*(grnimg + offset) = 255;
            else
            {
            	*(redimg + offset) = 0;
	  ...
	  /* Write to PPM file */
	  ppmWriteFileRGB(filename, redimg, grnimg, bluimg, ncols, 2*nrows);
...
/*********************************************************************
 * KLTWriteFeatureListToPPM with different colors
 */
  ...
  char *filename)
{
  ...
  for (i = 0 ; i < featurelist->nFeatures ; i++)
  {
    ...
    else if (featurelist->feature[i]->val == KLT_MAX_ITERATIONS )  {
      x = (int) (featurelist->feature[i]->x + 0.5);
      y = (int) (featurelist->feature[i]->y + 0.5);
      for (yy = y - 2 ; yy <= y + 2 ; yy++)
        for (xx = x - 2 ; xx <= x + 2 ; xx++)  
          if (xx >= 0 && yy >= 0 && xx < ncols && yy < nrows)  {
            ...
            *(grnimg + offset) = 0;
            *(bluimg + offset) = 0;
            //if (featurelist->feature[i]->val == KLT_MAX_ITERATIONS)
            //  {*(redimg + offset) = 200; *(grnimg + offset) = 200; *(bluimg + offset) = 200;}
            switch(i%6)
             {
         	case 0:
         		*(redimg + offset) = 255;
...
static FILE* _printSetupDispTxt(
  char *fname, 	/* Input: filename, or NULL for stderr */
  char *fmt,	/* Input: format (e.g., %5.1f or %3d) */
  char *format,	/* Output: format (e.g., (%5.1f,%5.1f)=%3d) */
  char *type)	/* Output: either 'f' or 'd', based on input format */
{
  ...
  /* Construct feature format */
  sprintf(format, "(%s)=(%s) ", fmt, fmt);
  //sprintf(format, "(%s,%s)=%%%df ", fmt, fmt, val_width);
  //sprintf(format, "(%s,%s)=%%f ", fmt, fmt);
  ...
  int nFeatures)
{
  ...
  assert(id == FEATURE_LIST || id == FEATURE_HISTORY || id == FEATURE_TABLE || id == DISPLACEMENT_LIST);
  ...
  switch (id)  {
     ...
     case DISPLACEMENT_LIST: fprintf(fp, "KLT Displacement List\n");    break;
  }
  ...
  switch (id)  {
     ...
     case DISPLACEMENT_LIST: fprintf(fp, "nFeatures = %d\n\n", nFeatures);    break;          
  }
  ...
  switch (id)  {
     ...
     case FEATURE_TABLE: fprintf(fp, "feature |          frame\n");   
       ...
       fprintf(fp, "\n");   
       break;
     case DISPLACEMENT_LIST: fprintf(fp, "feature | (left_id,right_id)=val\n");
       fprintf(fp, "--------+-");
       _printNhyphens(fp, width);
       fprintf(fp, "\n");   
       break;
