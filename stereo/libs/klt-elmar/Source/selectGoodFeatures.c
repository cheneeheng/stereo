...
typedef enum {SELECTING_ALL, REPLACING_SOME, SELECTING_SAME_IMAGE} selectionMode;
 ...
 *               Writes -1's into the remaining element.
 *
 * RETURNS
 ...
 * _minEigenvalue
 *
 * Given the three distinct element of the symmetric 2x2 matrix
 *                     [gxx gxy]
 *                     [gxy gyy],
  ...
  IppiRect *roi)
{
  ...
  // check ROI size
  if(xOffset < 0 || xOffset > (ncolsSrc-1))
  {
	  xOffset = 0;
	  roi->x = 0;
	  if(KLT_verbose >= 1)
	     KLTWarning("ROI x-coordinate out of range. Changing to %d.\n", xOffset);
  }
  if(yOffset < 0 || yOffset > (nrowsSrc-1))
  {
	  yOffset = 0;
	  roi->y = 0;
	  if(KLT_verbose >= 1)
	    KLTWarning("ROI y-coordinate out of range. Changing to %d.\n", yOffset);
  }
  if((ncols+xOffset) > ncolsSrc)
  {
	  ncols = ncolsSrc-xOffset;
	  roi->width = ncols;
	  if(KLT_verbose >= 1)
	    KLTWarning("ROI width out of range. Changing to %d.\n", ncols);
  }
  if((nrows+yOffset) > nrowsSrc)
  {
	  nrows = nrowsSrc-yOffset;
	  roi->height = nrows;
	  if(KLT_verbose >= 1)
	    KLTWarning("ROI height out of range. Changing to %d.\n", nrows);
  }
  ...
  /*if (mode == REPLACING_SOME &&
    ...
    else  */ //you have no computed images to refer to in patch mode
  //TODO: test functionality below
  if ( mode != SELECTING_ALL && roi->width==ncolsSrc && roi->height==nrowsSrc && //(mode == REPLACING_SOME || mode == SELECTING_SAME_IMAGE)  &&
		  /*!tc->patchMode &&*/ //eigentlich egal, weil sowieso auf tc->pyramid_last != NULL ueberprueft wird
		  tc->sequentialMode && tc->pyramid_last != NULL)  {
      floatimg = ((_KLT_Pyramid) tc->pyramid_last)->img[0];
      gradx = ((_KLT_Pyramid) tc->pyramid_last_gradx)->img[0];
      grady = ((_KLT_Pyramid) tc->pyramid_last_grady)->img[0];
    ...
    else
  {
    ...
    if (tc->smoothBeforeSelecting)  {
      ...
      _KLTPatchToFloatImage(img, ncolsSrc, nrowsSrc, roi, tmpimg);
    ...
    } else
    {
    	...
    	_KLTPatchToFloatImage(img, ncolsSrc, nrowsSrc, roi, floatimg);
    }
    /* Compute gradient of image in x and y direction */
    _KLTComputeGradients(floatimg, tc->grad_sigma, &gauss_kernel, &gaussderiv_kernel, &sigma_last, gradx, grady);
  }
     ...
     of the two eigenvalues of the Z matrix */
  {
    ...
    int pyramid_gaussderiv_hw = gaussderiv_kernel.width/2; //added by EM, cause the borderx/y becomes only calculated by the gauss_kernel.width and therefor are artefact-features at the borders
    int borderx = tc->borderx+pyramid_gaussderiv_hw;	/* Must not touch cols */
    int bordery = tc->bordery+pyramid_gaussderiv_hw;	/* lost by convolution */
    int x, y;
    int i;
...
/*#ifdef HAVE_IPP
    ...
    char *ipp_temp=malloc(nrows*ncols);
    int  buf_size_ipp;
    IppiSize roi={ncols-2*borderx,nrows-2*bordery};
    ippiMinEigenValGetBufferSize_32f_C1R(roi,3,5,&buf_size_ipp);
    char *ipp_buf=malloc(buf_size_ipp);
    ippiEigenValsVecs_8u32f_C1R((Ipp8u*)((_KLT_Pyramid) tc->pyramid_last)->img[0]+borderx,
         ncols,(Ipp32f*)ipp_temp,ncols*sizeof(float),ROI fehlt!!!, (IppiKernelType)ippKernelSobel,3,5,(Ipp8u*)ipp_buf);
    free(ipp_buf);
    IppiSize roi1={ncols,nrows};
    ippiThreshold_32f_C1IR((Ipp32f*)ipp_temp,ncols*sizeof(float),roi1,limit,ippCmpLess);
    ptr = pointlist;
    float *val_ptr_ipp=ipp_temp;
...
//#endif
  ...
  if(xOffset || yOffset)
  {
	  for(j=0;j<featurelist->nFeatures;j++)
	  {
		  if(featurelist->feature[j]->val>0)
		  {
			  featurelist->feature[j]->x += xOffset;
			  featurelist->feature[j]->y += yOffset;
		  }
	  }
  }
}
...
void _KLTExtSelectGoodFeatures(
  KLT_TrackingContext tc,
  KLT_PixelType *img,
  int ncols,
  int nrows,
...
{
	assert(width_dispersionFactor > 1);
	assert(height_dispersionFactor > 1);

	//int num_segments = width_dispersionFactor*height_dispersionFactor;
	int fl_help_nFeatures=fl->nFeatures/(width_dispersionFactor*height_dispersionFactor);//num_segments;
	int segmentWidth=roi->width/width_dispersionFactor;
	int segmentHeight=roi->height/height_dispersionFactor;
	int borderx=tc->borderx;
	int bordery=tc->bordery;

	//assert( (fl->nFeatures%num_segments)==0 );

	KLT_FeatureList fl_help = KLTCreateFeatureList(fl_help_nFeatures);
	int i, j, k;
	int fl_index=0;
	IppiRect roi_help;// = {roi->x, roi->y, roi->width+tc->borderx, roi->height+tc->bordery};
	roi_help.width=segmentWidth +2*borderx;

	for(j=0; j<width_dispersionFactor; j++)
	{
		if(j==0)
			roi_help.x=roi->x;
		else
			roi_help.x=roi->x+j*segmentWidth-borderx;

		if(j==width_dispersionFactor-1)
			roi_help.width=roi->x+roi->width - roi_help.x;

		roi_help.height=segmentHeight +2*bordery;

		for(k=0; k<height_dispersionFactor; k++)
		{
			if(k==0)
				roi_help.y=roi->y;
			else
				roi_help.y=roi->y+k*segmentHeight-bordery;

			if(k==height_dispersionFactor-1)
				roi_help.height=roi->y+roi->height - roi_help.y;

			//fprintf(stderr, "Before run %d\n", j*height_dispersionFactor+k);
			_KLTSelectGoodFeatures(tc, img, ncols, nrows, fl_help, SELECTING_ALL, &roi_help);
			//fprintf(stderr, "After run %d\n", j*height_dispersionFactor+k);
			for(i=0; i<fl_help_nFeatures; i++)
				KLTMoveFeature(fl_help->feature[i], fl->feature[fl_index++]);

			//KLTWriteFeatureList(fl, "feat_left.txt", "%3d");
			//char help;
			//scanf("%c", &help);
		}
	}

    //fprintf(stderr, "DONE - fl_index : %d\n", fl_index);

	for(i=fl_index; i<fl->nFeatures; i++)
		fl->feature[i]->val = -99;
...
void KLTSelectGoodFeatures(
  ...
  KLT_FeatureList fl)
{
  IppiRect roi={0,0,ncols,nrows};

  KLTExtSelectGoodFeatures(tc, img, ncols, nrows, fl, &roi, 1, 1);
}



/*********************************************************************
 * KLTExtSelectGoodFeatures
 *
 * Main routine, visible to the outside.  Finds the good features in
 * an image's ROI.
 *
...

{
  ...
  if(width_dispersionFactor < 2 && height_dispersionFactor < 2)
  	_KLTSelectGoodFeatures(tc, img, ncols, nrows, fl, SELECTING_ALL, roi);
  else
  	_KLTExtSelectGoodFeatures(tc, img, ncols, nrows, fl, /*SELECTING_ALL,*/ roi, width_dispersionFactor, height_dispersionFactor);
...
/*********************************************************************
 * KLTExtSelectGoodFeaturesInSameImage
 *
 * Main routine, visible to the outside.  Finds the good features in
 * an image which subimages have already been computed.
 *
 * INPUTS
 * tc:	Contains parameters used in computation (size of image,
 *        size of window, min distance b/w features, sigma to compute
  ...
  IppiRect *roi)
{
  ...
  _KLTSelectGoodFeatures(tc, NULL, ncols, nrows,
                         fl, SELECTING_SAME_IMAGE, roi);
  ...
  KLT_FeatureList fl)
{
  ...
  KLTExtReplaceLostFeatures(tc, img, ncols, nrows, fl, &roi);
}

/*********************************************************************
 * KLTExtReplaceLostFeatures
 *
 * Main routine, visible to the outside.  Replaces the lost features
 * in an image's ROI.
 *
...

{
  ...
  if (nLostFeatures > 0)
    _KLTSelectGoodFeatures(tc, img, ncols, nrows, fl, REPLACING_SOME, roi);
