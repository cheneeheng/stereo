...
void* stereo(void* arg)
{
  ...
  int sgbmWinSize = 9; //filter window size
...
/*
  for(int j=0;j<num_points;j++)
  {
    ...
    int j = 3;
    point = point3D.at<cv::Vec3f> (fl->feature[j]->y,fl->feature[j]->x);
    float r = sqrt(Sqr(point[0])+Sqr(point[1])+Sqr(point[2]));
    std::cout << point[0]/r << std::endl;
    std::cout << point[1]/r << std::endl;
    std::cout << point[2]/r << std::endl;
    std::cout << "" << std::endl;

    point2[0]=fl->feature[j]->x - P1.at<double>(0,2);
    point2[1]=fl->feature[j]->y - P1.at<double>(1,2);
    point2[2]=P1.at<double>(0,0);
    r=sqrt(Sqr(point2[0])+Sqr(point2[1])+Sqr(point2[2]));

    std::cout << point2[0]/r << std::endl;
    std::cout << point2[1]/r << std::endl;
    std::cout << point2[2]/r << std::endl;
    std::cout << "" << std::endl;
*/
  ...
  cv::Mat tmp_img_right, disp2;
  int flag_vec1 = 400;
  float x,y;
  std::vector<float> x2(4),y2(4);
...
//            window.fillEllipse(fl->feature[k]->x-10, fl->feature[k]->y-10, 20, 20, "red");
      ...
      for(int j=0;j<num_points;j++)
      {
        ...
        x = fl->feature[j]->x; y = fl->feature[j]->y;
        //std::cout << fl->feature[j]->val << std::endl;
...
*/
      ...
      vgps.find_pose(vec_2,vec_1,Depth1,Trans.R,Trans.T,1500);
...
/*
      ...
      for(int j=0;j<num_points;j++)
      {
        ...
        //vec_2[1][j]=point[1];
        //vec_2[2][j]=point[2];   
        vec_2[0][j]=fl->feature[j]->x - P1.at<double>(0,2);
        vec_2[1][j]=fl->feature[j]->y - P1.at<double>(1,2);
        vec_2[2][j]=P1.at<double>(0,0);
        Depth2[j]=sqrt(Sqr(vec_2[0][j])+Sqr(vec_2[1][j])+Sqr(vec_2[2][j]));
        vec_2[0][j]/=Depth2[j];
        vec_2[1][j]/=Depth2[j];
...
/*
      ...
      vgps.find_pose(vec_2,vec_1,Depth1,Trans.R,Trans.T,2000);
      Trans2.T=Trans.R.t()*Trans.T;

      std::cout << "Flag : " << flag_vec1 << std::endl;
      std::cout << "Validate : " << vgps.validate_set(vec_2,vec_1,Trans.R,Trans.T) << std::endl;
